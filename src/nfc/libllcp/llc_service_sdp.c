...
#else
#define LLC_SDP_MSG(priority, message) llcp_log_log (LOG_LLC_SDP, priority, "(%p) %s", xTaskGetCurrentTaskHandle (), message)
#define LLC_SDP_LOG(priority, format, ...) llcp_log_log (LOG_LLC_SDP, priority, "(%p) " format, xTaskGetCurrentTaskHandle (), __VA_ARGS__)
#endif
...
#else
  llc_up   = connection->llc_up;
  llc_down = connection->llc_down;
