...
#define LLC_SERVICE_LLC_MSG(priority, message) llcp_log_log (LOG_LLC_SERVICE_LLC, priority, "(%p) %s", xTaskGetCurrentTaskHandle(), message)
#define LLC_SERVICE_LLC_LOG(priority, format, ...) llcp_log_log (LOG_LLC_SERVICE_LLC, priority, "(%p) " format, xTaskGetCurrentTaskHandle(), __VA_ARGS__)
#define INC_MOD_16(x) x = (x + 1) % 16

void llc_service_llc_thread_cleanup(void *arg)
{
  /*
   * Beware:
  ...
  if (link->llc_up != (xQueueHandle) -1)
    mq_close(link->llc_up);

  if (link->llc_down != (xQueueHandle) -1)
    mq_close(link->llc_down);

  link->llc_up   = (xQueueHandle) -1;
  link->llc_down = (xQueueHandle) -1;
}

void llc_service_llc_thread(void *arg)
{
  struct llc_link *link = (struct llc_link *)arg;
  xQueueHandle llc_up, llc_down;
...
#else
  if (llc_up == (xQueueHandle) -1)
    LLC_SERVICE_LLC_LOG(LLC_PRIORITY_ERROR, "mq_open(%s)", link->mq_up_name);
  if (llc_down == (xQueueHandle) -1)
    LLC_SERVICE_LLC_LOG(LLC_PRIORITY_ERROR, "mq_open(%s)", link->mq_down_name);
#endif
  ...
  for (;;) {
    ...
    res = xQueueReceive(llc_up, &buffer2, portMAX_DELAY);
    memcpy(buffer,buffer2.data,sizeof(char)*buffer2.len);
    ...
    switch (pdu->ptype) {
      ...
      case PDU_AGF:
        ...
        while (*p) {
		  ...
		  llc_queue buffer2;
		  buffer2.len = pdu_pack(pdu, buffer2.data, sizeof(buffer2));
...
spawn_logical_data_link:
        ...
        if (mq_send(connection->llc_up, &buffer2, res, 0) < 0) {
...
#else
        xTaskHandle thread = link->datagram_handlers[i]->thread;
#endif
...
#else
        xTaskHandle thread = link->transmission_handlers[i]->thread;
#endif
...
#else
                // if queue is full then it should be wait until it is not full??
                while(prvIsQueueFull(link->transmission_handlers[i]->llc_up)){
                	vTaskDelay(1);
                }
                reply = pdu_new_rr(link->transmission_handlers[i]);
#endif
