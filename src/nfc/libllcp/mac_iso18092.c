...
void llcp_threadslayer(xTaskHandle thread);
#endif
...
#else
   	  if (xQueueSend(link->llc_link->llc_up, (char *) buffer, 0) != pdPASS)
#endif
      {
...
#else
    len = xQueueReceive(link->llc_link->llc_down, (char *) buffer, (portTickType)2);
#endif
...
#else
  if ((link->exchange_pdus_thread = malloc(sizeof(xTaskHandle))) == NULL) {
    MAC_LINK_MSG(LLC_PRIORITY_FATAL, "Cannot allocate PDU exchanging thread structure");
    return -1;
  }
#endif
...
#else
  assert((link->exchange_pdus_thread == NULL) || (*link->exchange_pdus_thread != xTaskGetCurrentTaskHandle()));
#endif
