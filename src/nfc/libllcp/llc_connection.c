...
extern void llcp_threadslayer(xTaskHandle thread);
#endif
...
#else
    res->llc_up   = (xQueueHandle)-1;
    res->llc_down = (xQueueHandle)-1;
...
#else
  connection->llc_up = xQueueCreate( 2, 3 + connection->local_miu );
#endif

#ifndef STM32F40_41xxx
  if (connection->llc_up == (mqd_t) - 1) {
#else
	  if (connection->llc_up == (xTaskHandle)-1) {
#endif
...
#else
  connection->llc_down = xQueueCreate( 2, 3 + connection->remote_miu );
#endif

#ifndef STM32F40_41xxx
  if (connection->llc_down == (mqd_t) - 1) {
#else
    if (connection->llc_down == (xQueueHandle)-1) {
#endif
...
#else
	assert(connection->thread == xTaskGetCurrentTaskHandle());
#endif
...
#else
  if (xQueueSend(connection->llc_down, &buffer2, 0) != pdPASS)
#endif
  {
...
#else
  res = xQueueReceive(connection->llc_up, (char *) buffer, 0);
#endif
  if (res != pdPASS) {
...
#else
	if (connection->thread == xTaskGetCurrentTaskHandle())
#endif
  {
...
#else
  if (connection->llc_up != (xQueueHandle)-1)
    vQueueDelete(connection->llc_up);
  if (connection->llc_down != (xQueueHandle)-1)
	vQueueDelete(connection->llc_down);
/*
  if (connection->mq_up_name)
    mq_unlink(connection->mq_up_name);
  if (connection->mq_down_name)
