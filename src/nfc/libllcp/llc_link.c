...
llc_link_activate(struct llc_link *link, uint8_t flags, const uint8_t *parameters, size_t length)
{
...
#else
  link->llc_up   = xQueueCreate(2,3 + link->local_miu);
...
#else
	  link->llc_down = xQueueCreate(2,3 + link->remote_miu);
  ...
  portBASE_TYPE ret = xTaskCreate(llc_service_llc_thread,( signed char * )"LLC TASK",1024,&link,0x0F,&link->thread);
  if ( ret == pdPASS) {
	#if defined(HAVE_DECL_PTHREAD_SET_NAME_NP) && HAVE_DECL_PTHREAD_SET_NAME_NP
	  pthread_set_name_np(link->thread, "LLC Link");
	#endif
...
llc_link_send_pdu(struct llc_link *link, const struct pdu *pdu)
{
  ...
  if (mq_send(link->llc_down, buffer2, buffer2.len, 0) < 0) {
...
#else
	if (link->thread == xTaskGetCurrentTaskHandle())
#endif
  {
    /*
...
#else
      link->thread = (xTaskHandle)NULL;  // XXX Could we assume pthread_t struct is a pointer ?
#endif
    }
  }
